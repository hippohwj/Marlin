...
#include "FaultToleranceWorkload.h"
#include "ITxn.h"
...
uint32_t FaultToleranceWorkload::granule_num_per_node = 32 * 4;

FaultToleranceWorkload::FaultToleranceWorkload(ARDB *db, YCSBConfig *config) : ScaleoutWorkload(db, config) {
}

void FaultToleranceWorkload::Init() {
  LOG_INFO("Fault-tolerance Init...")
...
void FaultToleranceWorkload::InitComplement() {
  LOG_INFO("Fault-tolerance Complement Init...")
  M_ASSERT(config_->migrate_cache_enable_ == 0, "only support migration_cache_enable_ = 0");
  GenLockTable();
  GenQueries();
}

void FaultToleranceWorkload::GenLockTable() {
    ...
    for (int i = 0; i < migr_gran_num_per_node; i++) {
      uint64_t start_gran_id = granule_num_per_node * g_scale_node_id;
...
void FaultToleranceWorkload::GenQueries() {
    M_ASSERT(g_migr_postheat_enable, "Fault-tolerance workload can only use g_migr_postheat_enable");
    LOG_INFO("Fault-tolerance Workload Generate Queries...")
